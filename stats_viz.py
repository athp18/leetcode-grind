...
def create_svg(counts, percentages):
    ...
    for i, (category, count) in enumerate(counts.items()):
        ...
        # Category label
        label = ET.SubElement(svg, 'text', x='140', y=str(y + bar_height / 2 + 5), fill='black', font_size='14')
        label.text = category.capitalize()
        
        # Percentage label
        pct_label = ET.SubElement(svg, 'text', x=str(150 + bar_length + 10), y=str(y + bar_height / 2 + 5), fill='black', font_size='14')
        pct_label.text = f"{percentages[category]:.1f}%"
        
        # Count label inside the bar
        count_label = ET.SubElement(svg, 'text', x='155', y=str(y + bar_height / 2 + 5), fill='white', font_size='14')
...
"""
    ...
    if '## Progress' in readme_content:
        ...
        after_progress = readme_content[start:]
        next_section = after_progress.find('## ', 2)
        if next_section != -1:
            end = start + next_section
        else:
            end = len(readme_content)
        # Replace the existing Progress section
        new_content = readme_content[:start] + progress_section + readme_content[end:]
    else:
        # Append the Progress section at the end
